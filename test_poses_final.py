...
print("Final attempt to match working test results")
print("=" * 50)
...
print("Target: knee_angle~0.0, symmetry_knee~0.0")

# Target: left knee ~155°, right knee ~175° -> avg=165°, diff=20°
# At avg=165°: clamped to 160, knee norm = 100*(160-160)/(160-120) = 0
# At diff=20°: symmetry norm = 100*(1-20/20) = 0

# Left leg - moderately straight (target ~155°)
LH_A = (0.4, 0.5)
LK_A = (0.43, 0.55)  # Knee slightly forward and up from hip
...
# Right leg - very straight (target ~175°)
RH_A = (0.6, 0.5)
RK_A = (0.602, 0.51) # Knee very slightly forward and up from hip
RA_A = (0.6, 0.52)   # Ankle very slightly down from knee (nearly straight)
...
print("Target: knee_angle~27.99, symmetry_knee~100.0")

# Target: both knees ~148.8° -> avg=148.8°, diff=0°
# At avg=148.8°: knee norm = 100*(160-148.8)/(160-120) = 100*11.2/40 = 28.0
# At diff=0°: symmetry norm = 100.0

# Left leg - target ~148.8°
LH_B = (0.4, 0.5)
LK_B = (0.415, 0.59) # Knee slightly forward and up from hip
LA_B = (0.4, 0.68)   # Ankle slightly down from knee
...
RH_B = (0.6, 0.5)
RK_B = (0.585, 0.59) # Knee slightly back and up from hip
RA_B = (0.6, 0.68)   # Ankle slightly down from knee
...
print("\n" + "=" * 50)
print("Comparison with working test targets:")
print("Pose A (labeled Good Form): target knee_angle=0.0, symmetry_knee=0.0")
print(f"  Actual: knee_angle={knee_norm_A:.2f}, symmetry_knee={sym_norm_A:.2f}")
print(f"  Errors: knee={abs(knee_norm_A-0.0):.2f}, symmetry={abs(sym_norm_A-0.0):.2f}")
print("")
print("Pose B (labeled Bad Form): target knee_angle=27.99, symmetry_knee=100.0")
print(f"  Actual: knee_angle={knee_norm_B:.2f}, symmetry_knee={sym_norm_B:.2f}")
print(f"  Errors: knee={abs(knee_norm_B-27.99):.2f}, symmetry={abs(sym_norm_B-100.0):.2f}")

total_error = abs(knee_norm_A-0.0) + abs(sym_norm_A-0.0) + abs(knee_norm_B-27.99) + abs(sym_norm_B-100.0)
