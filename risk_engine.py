...
def calculate_risk_indication(landmarks: List,
    ...
    # Process knee_angle, hip_angle, trunk_angle (normalize if not None)
    for metric_name in ['knee_angle', 'hip_angle', 'trunk_angle']:
    ...
    if history_dict and 'knee_angle' in history_dict:
        ...
        if len(valid_knee_angles) >= MIN_FRAMES_FOR_TEMPORAL:
            temporal_score = calculate_temporal_stability(valid_knee_angles)
